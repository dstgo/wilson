...
// Struct validates a struct by checking the specified struct fields against the corresponding validation rules.
...
func Struct(structPtr interface{}, lang string, fields ...*FieldRules) error {
	return StructWithCtx(nil, lang, structPtr, fields...)
}

// StructWithCtx validates a struct with the given context.
// The only difference between StructWithCtx and Struct is that the former will
// validate struct fields with the provided context.
// Please refer to Struct for the detailed instructions on how to use this function.
func StructWithCtx(ctx context.Context, lang string, structPtr interface{}, fields ...*FieldRules) error {
	...
	for i, fr := range fields {
		...
		var err error
		if ctx == nil {
			err = Validate(lang, fv.Elem().Interface(), fr.rules...)
		} else {
			err = ValidateWithContext(ctx, lang, fv.Elem().Interface(), fr.rules...)
		}
		if err != nil {
			var ie InternalError
			if errors.As(err, &ie) && ie.InternalError() != nil {
				return err
			}
			if ft.Anonymous {
			...
			errs = append(errs, Err{getErrorFieldName(lang, ft), err})
		}
	}
...
func getErrorFieldName(lang string, f *reflect.StructField) string {
	tag, ok := f.Tag.Lookup(LabelTag)
	if ok && translator != nil {
		return translator.Get(lang, tag)
	}
	return f.Name
}
