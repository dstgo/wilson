...
func (r MapRule) ValidateWithContext(ctx context.Context, lang string, m interface{}) error {
	...
	for _, kr := range r.keys {
		...
		if kv := reflect.ValueOf(kr.key); !kt.AssignableTo(kv.Type()) {
			...
			err = Validate(lang, vv.Interface(), kr.rules...)
		} else {
			err = ValidateWithContext(ctx, lang, vv.Interface(), kr.rules...)
		}
		if err != nil {
			if ie, ok := err.(InternalError); ok && ie.InternalError() != nil {
				return err
			}
			errs = append(errs, Err{getErrorKeyName(kr.key), err})
		}
		if !r.allowExtraKeys {
	...
	if !r.allowExtraKeys {
		for key := range extraKeys {
			errs = append(errs, Err{getErrorKeyName(key), ErrKeyUnexpected})
		}
	}
