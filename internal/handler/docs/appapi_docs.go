// Package docs Code generated by swaggo/swag at 2023-10-28 10:00:47.4460793 +0800 CST m=+3.734990101. DO NOT EDIT
package docs
...
const docTemplateappapi = `{
    ...
    "paths": {
        ...
        "/auth/refresh": {
            "get": {
                "description": "[guest]\ncarry refresh token in query params, access token in header\nif refresh-token expired , Refresher will not refresh token [4012]\nelse if access-token has expired after delay duration, Refresher will not refresh token [4012]\nelse if access-token has expired before delay duration, Refresher will issue a new access-token [2005]\nelse if access-token has not expired, Refresher will renewal the 1/10 access-token ttl per time  [2005]\nelse if access-token has not expired, and ttl \u003e= 2 * conf.JwtConf.Exp, Refresher will not refresh token [4013]",
                "consumes": [
                    "application/json"
                ],
                "produces": [
