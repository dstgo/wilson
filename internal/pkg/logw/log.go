...
func NewLogger(conf *conf.LogConf) (*LoggerW, error) {
	...
	infoHook := NewLevelLoggerH(infoLog, NewTextFormatter(), 40960, logrus.DebugLevel, logrus.TraceLevel, logrus.InfoLevel)

	errorHook := NewLevelLoggerH(errorLog, NewTextFormatter(), 40960, logrus.WarnLevel, logrus.ErrorLevel, logrus.PanicLevel, logrus.FatalLevel)
