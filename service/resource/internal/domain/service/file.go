...
import (
	...
	"github.com/dstgo/wilson/service/resource/internal/utils"
)
...
func (u *File) PrepareUploadFile(ctx kratosx.Context, req *types.PrepareUploadFileRequest) (*types.PrepareUploadFileReply, error) {
	...
	// 校验文件大小
	if size := utils.GetKBSize(limit.MaxSize); size < req.Size {
		return nil, errors.ExceedMaxSizeError()
	}

	// 校验文件类型
	tp := utils.GetFileType(req.Name)
	if !lo.Contains(limit.Accepts, tp) {
