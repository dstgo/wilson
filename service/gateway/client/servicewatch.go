	...
	existed := func() bool {
		...
		if ok {
			...
			if len(ws.selectedInstances) > 0 {
				...
				err := applier.Callback(ws.selectedInstances)
				if err != nil {
					panic(err)
				}
		...
		func() {
			...
			if err != nil {
				panic(fmt.Sprintf("Failed to do initialize services discovery on endpoint: %s, err: %+v, the watch process will attempt asynchronously", endpoint, err))
			}
