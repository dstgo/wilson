...
import (
	...
	"github.com/dstgo/wilson/service/daemon/internal/app"
	"github.com/dstgo/wilson/service/daemon/internal/conf"
)
...
var service = cli.NewCLI(&cli.Options{
	...
	Description: "daemon service for wilson framework",
	StartFn:     Start,
})
...
func RegisterServer(c config.Config, hs *thttp.Server, gs *grpc.Server) {
	...
	// watch config
	c.ScanWatch("daemon", func(value config.Value) {
		if err := value.Scan(cfg); err != nil {
			log.Printf("daemon config format error: %s", err.Error())
		}
	})
